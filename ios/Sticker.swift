...
class Sticker {
     ...
     */
     ...
     /**
 *  Initializes a sticker with an image file URL and emojis.
 *
 *  - Parameter fileUrl: URL of the image as a String. Must be either png or webp.
 *  - Parameter emojis: emojis associated with this sticker.
 */
init?(contentsOfFileUrl fileUrl: String, emojis: [String]?) {
    ...
    do {
        self.imageData = try ImageData.imageDataIfCompliant(rawData:unwrappedData, extensionType: .png, isTray: false)
        self.emojis = try StickerEmojis.canonicalizedEmojis(rawEmojis: emojis)
    } catch {
        print("Error initializing Sticker: \(error)")
        return nil
    }
}
